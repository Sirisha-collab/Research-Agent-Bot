...
import { api } from "./api";
import type { ChatTurn, DocumentDetail, DocumentSummary, Health, Source } from "./types";
...
>>>>>>> Stashed changes
  ...
  function openSourcePage(source: Source) {
    const known = docs.find((d) => d.doc_id === source.doc_id);
    setPdf({
      docId: source.doc_id,
      title: source.doc_title || known?.title || "Source PDF",
      page: source.page > 0 ? source.page : 1,
      pageCount: known?.page_count ?? (active?.doc_id === source.doc_id ? active.page_count : 0),
      label: source.label,
    ...
    <div className="mx-auto max-w-6xl px-5 py-10">
      <header className="relative pb-4">
        ...
        <div
          className={clsx(
            "px-5 py-8",
            pdf ? "max-w-none sm:mr-[30rem] lg:mr-[34rem]" : "mx-auto max-w-6xl",
          )}
        >
          ...
          <div className="mt-5 grid items-start gap-6 lg:grid-cols-[20rem_1fr]">
            <div className="space-y-5 lg:sticky lg:top-8">
              ...
              <div
                className={clsx(
                  "mt-4 grid gap-6",
                  pdf ? "xl:grid-cols-[18rem_1fr]" : "lg:grid-cols-[20rem_1fr]",
                )}
              >
                        ...
                        <div className="border-t border-rule px-5 py-5 sm:px-6">
                          ...
                          <div className="mt-4 border-t border-rule pt-4">
                            <div className="mb-2 flex items-center justify-between gap-3">
                              ...
                              {active.summary && <CopyButton text={active.summary} />}
                            </div>
              ...
              {pdf && (
                <PdfViewer
                  target={pdf}
                  onClose={() => setPdf(null)}
                  onPageChange={(page) => setPdf({ ...pdf, page })}
                />
              )}
            </div>
            );
